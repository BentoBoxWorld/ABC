 ...
 *
 */
public class AdminMapCommand extends CompositeCommand {

    private Long price;

    public AdminMapCommand(Addon addon, CompositeCommand adminCommand) {
        super(addon, adminCommand, "map");
    }

    @Override
    public void setup() {
        setPermission("admin.abc.map");
        setOnlyPlayer(false);
        setParametersHelp("abc.commands.admin.map.parameters");
        setDescription("abc.commands.admin.map.description");
        new AdminMapNameCommand(getAddon(), this);
    }
